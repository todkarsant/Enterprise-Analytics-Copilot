...
"""Process-isolated/resumable Spider P0-P5 benchmark runner.

The frozen benchmark policies, dataset, evaluator, metrics, and model configuration
are unchanged. Each (case, policy) execution runs in a separate OS process. The
parent enforces a hard wall-clock deadline with subprocess.run(timeout=...), so a
hung HTTP client, SQLite execution, evaluator, or Python cleanup path cannot hold
the whole partition indefinitely. Completed traces are checkpointed after every
policy and timeout/error traces remain in the denominator.
"""
...
def worker(args: argparse.Namespace) -> None:
    ...
    example = next(
        x for x in dataset.examples
        if x.question == args.question and x.db_id == args.db_id
    )
    trace = env.run(example, args.policy)
    print(json.dumps(asdict(trace), separators=(",", ":")), flush=True)


def run_policy_isolated(
    questions: Path,
    database_dir: Path,
    example: Example,
    policy: str,
    ...
    try:
        ...
        return Trace(
            ...
            policy,
            latency_ms=elapsed,
            termination_reason="policy_process_timeout",
            error=f"hard_process_timeout_{timeout_seconds}s: {exc}",
        )
    ...
    if completed.returncode != 0:
        ...
        return Trace(
            ...
            policy,
            latency_ms=elapsed,
            termination_reason="policy_process_error",
            error=f"worker_exit_{completed.returncode}: {stderr[-1000:]}",
        )

    try:
        payload = json.loads((completed.stdout or "").strip().splitlines()[-1])
        trace = Trace(**payload)
        # Include process startup/IPC overhead in the observed wall-clock metric.
        ...
        return Trace(
            ...
            policy,
            latency_ms=elapsed,
            termination_reason="worker_protocol_error",
            error=f"invalid_worker_output: {exc}; stdout={(completed.stdout or '')[-1000:]}",
        )
...
def parent(args: argparse.Namespace) -> None:
    ...
    for index, example in enumerate(examples):
        print(
            f"CASE {index + 1}/{len(examples)} db={example.db_id} question={example.question!r}",
            flush=True,
        )
        for policy in POLICIES:
            ...
            trace = run_policy_isolated(
                args.questions,
                args.database_dir,
                example,
                policy,
                args.policy_timeout_seconds,
            )
            row = asdict(trace)
            traces.append(row)
            done.add(key)
            ...
            print(
                f"  {policy}: END termination={trace.termination_reason} "
                f"latency_ms={trace.latency_ms:.1f} llm_calls={trace.llm_calls} error={trace.error!r}",
                flush=True,
            )
    ...
    payload = {
        ...
        "secondary_custom_results": summarize(
            [Trace(**r) for r in traces], "custom_execution_correct"
        ),
        "official_spider_execution": None,
        "traces": traces,
        "runtime_controls": {
            "policy_timeout_seconds": args.policy_timeout_seconds,
            "isolation": "one OS subprocess per case-policy execution",
            "checkpoint": str(args.checkpoint),
            "resume_enabled": True,
        },
        "warning": (
            "Official Spider execution evaluation is primary. Runtime process timeouts "
            "and worker errors are recorded as failures and are not silently excluded."
        ),
    }
    official = evaluate_traces(
        payload,
        args.database_dir,
        args.tables_file,
        args.spider_eval_dir,
    )
